...
glm::mat4 KeyframeCollection::interpolateTransformation( float time ) const
{
    ...
    if( !m_keyframes.empty() )
    {
        ...
        glm::quat R = glm::slerp(result[0].second.getOrientation(), result[1].second.getOrientation(), factor);
        glm::vec3 T = glm::lerp(result[0].second.getTranslation(), result[1].second.getTranslation(), factor);
        glm::vec3 S = glm::lerp(result[0].second.getScale(), result[1].second.getScale(), factor);

        //Build a matrix transformation from the orientation, translation and scale components
        return GeometricTransformation(T, R, S).toMatrix();
    }
    else
    {
        return glm::mat4(1.0);
